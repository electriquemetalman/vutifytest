<template>
  <v-dialog v-model="dialog" max-width="400px">
    <template v-slot:activator="{ on, attrs }">
        <v-btn dark v-bind="attrs" v-on="on" class="success" >Add new product</v-btn>
    </template>    
        <v-card>
            <v-card-title>
            ...
            <v-card-text>
                <v-form class="px-3">
                    ...
                    <v-btn  class="blue white--text mx-0 mt-3" @click="submit" :loading="loading">Add product</v-btn>
                </v-form>
            </v-card-text>
